 ...
 * ```typescript
 * const data = await db.query.table.findFirst().catch((error) =>
 *   throw toInternalError(error, 'Failed to get data', {
...
import { toInternalError } from 'fpp/server/api/custom-error';
  ...
  getRoomStats: publicProcedure
    ...
    .query(async ({ ctx: { db }, input: { roomId } }) => {
      ...
      const room = await db.query.rooms.findFirst({
        ...
        throw toInternalError(error, 'Failed to query room', {
          component: 'roomRouter',
          action: 'getRoomStats',
          extra: { roomId },
          severity: 'high',
      ...
      const response = await fetch(`${env.ANALYTICS_URL}/room/${roomId}/stats`).catch((error) => {
        throw toInternalError(error, 'Failed to fetch analytics', {
          component: 'roomRouter',
          action: 'getRoomStats',
...
2. Add `import { toInternalError } from 'fpp/server/api/custom-error'`
3. Remove try-catch blocks
4. Replace `captureError + throw new TRPCError` with `.catch(() => throw toInternalError(...))`
...
- [ ] All routers import toInternalError
...
2. `src/server/api/routers/analytics.router.ts` - Remove captureError, use toInternalError
3. `src/server/api/routers/config.router.ts` - Remove captureError, use toInternalError
4. `src/server/api/routers/contact.router.ts` - Remove captureError, use toInternalError
5. `src/server/api/routers/landingpage.router.ts` - Remove captureError, use toInternalError
6. `src/server/api/routers/roadmap.router.ts` - Remove captureError, use toInternalError
7. `src/server/api/routers/room.router.ts` - Remove captureError, use toInternalError (3 procedures)
8. `src/server/api/routers/sentry.router.ts` - Remove captureError, use toInternalError
...
const data = await db.query.table.findFirst({
  ...
  throw toInternalError(error, 'Failed to query database', {
    component: 'exampleRouter',
...
const response = await fetch(url, options).catch((error) => {
  throw toInternalError(error, 'Failed to fetch external API', {
    component: 'exampleRouter',
    action: 'procedureName',
    extra: { url },
    severity: 'high',
...
const conflict = await db.query.rooms.findFirst({
  ...
  throw toInternalError(error, 'Failed to check name conflict', {
    component: 'roomRouter',
    action: 'updateRoomName',
...
await db.update(rooms)
  ...
  .where(eq(rooms.id, roomId))
  .catch((error) => {
    throw toInternalError(error, 'Failed to update room name', {
      component: 'roomRouter',
      action: 'updateRoomName',
      extra: { roomId, newName },
      severity: 'high',
...
- [ ] toInternalError creates correct error structure
