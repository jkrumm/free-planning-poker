...
> **Plain-log transport on web is OTLP, not stdout.** §3 classifies operator narration (fail-open warnings) as a plain log — which silently assumes a stdout aggregator. That holds for fpp-server and analytics (Pino → ClickStack on the VPS) but **not** for the Vercel-hosted web app: its stdout lands only in Vercel's platform logs, which our ClickStack/HyperDX stack never reads. So on web, narration goes through a facade verb `log.warn`/`log.info` that dual-writes Pino stdout (Logdy locally, Vercel platform logs in prod) **and**, server-side only, a plain OTLP log record (no `event.name`, so it stays distinct from a `recordEvent` occurrence). In the browser the OTLP emit is skipped — the HyperDX SDK's `consoleCapture` already forwards Pino's console output, and emitting explicitly would double-log. Raw `logger` (`fpp/utils/logger`) is ESLint-banned outside the facade to keep this invariant. The web `LoggerProvider` uses `SimpleLogRecordProcessor` (not `BatchLogRecordProcessor`): Vercel functions can suspend after the response before a batch flushes, dropping these low-volume server-side records. `@opentelemetry/instrumentation-pino` was evaluated as an auto-bridge and rejected — it is incompatible with `@vercel/otel` (the trace SDK we use) and has open Next.js export-failure issues; the explicit facade emit is the robust path.
