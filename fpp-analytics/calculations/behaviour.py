...
def extract_sources(sources: dict, sources_mapping: list) -> dict:
...
def calc_behaviour() -> dict:
    ...
    sources = extract_sources(sources_raw, [
        ["Teams", r"teams\."],
    ...
    df_events = pl.read_parquet(
        data_dir / "fpp_events.parquet",
        columns=["event"]
    )

    # Amount of events for each event
    events = dict(
        df_events.group_by("event")
        .len()
        .iter_rows()
    )

    # Load vote and room data for room popularity
    df_votes = pl.read_parquet(
        data_dir / "fpp_votes.parquet",
        columns=["room_id"]
    )

    df_rooms = pl.read_parquet(
        data_dir / "fpp_rooms.parquet"
    )
